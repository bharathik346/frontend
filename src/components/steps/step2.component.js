...
class Step2 extends Component {
    ...
    handleClick = (value) => {
        ...
        if(value ===  'next'){
            ...
            receiverAddress.name = this.state.name;
            receiverAddress.address = this.state.address;
            receiverAddress.city = this.state.city;
            receiverAddress.state = this.state.state;
            receiverAddress.zip = this.state.zip;
            if(!isNil(this.state.name ) && !isNil(this.state.state ) && !isNil(this.state.city ) && !isNil(this.state.address ) && !isNil(this.state.zip ) ){
                ...
                this.props.onAction({currentStep:3 , receiverAddress})
            }else{
                this.setState({isError:true})
            }
            
        }else{
            this.props.onAction({currentStep:1})
        }
    ...
    componentWillMount() {
        ...
        this.setState({
            name: get(payload,'to.name'),
            address:get(payload,'to.address'),
            city:get(payload,'to.city'),
            state:get(payload,'to.state'),
            zip:get(payload,'to.zip'),
        })
    }
    ...
    render() {
        ...
        return(
            <div>
                <div class="card">
                    <div class="card-body">
                        <h5 class="card-title"> Enter the receiver's address  </h5>
                        <div>
                            {this.getContent()}
                        </div>
